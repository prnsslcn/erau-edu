...
export default function ChapterVideos({ videos }: { videos: VideoItem[] }) {
  ...
  return (
    <div className="space-y-3">
      {/* 진도 + 토글 + 펼침(블라인드)을 하나의 뉴모피즘 박스 안에 */}
      <div className="neu-raised rounded-2xl px-5 py-3.5">
        ...
        {/* 박스 안에서 블라인드처럼 펼쳐지는 Clip 목록 */}
        {multi && (
          <div
            className="overflow-hidden transition-[max-height] duration-500 ease-out"
            style={{
              maxHeight: open ? (chipsRef.current?.scrollHeight ?? 1000) : 0,
            }}
          >
            <div ref={chipsRef} className="flex flex-wrap gap-2 pt-3">
              {videos.map((v, i) => (
                <button
                  key={v.id}
                  onClick={() => setActive(i)}
